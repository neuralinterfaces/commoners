...
const onWindowReady = win => {
  win.addEventListener('message', async ev => {
    ...
    if (command === 'ping')
      return win.send('pong', { request: payload, response: performance.now() })
...
READY.then(({ windows }) => {
  ...
  if (Object.keys(windows).length === 1 && windows.main)
    return `<p>Cannot open windows from a secondary window.</p>`
