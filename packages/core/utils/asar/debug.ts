...
function validateAsarStructure(asarPath: string): { isValid: boolean; error?: string } {
  ...
  try {
    ...
    if (headerSize !== 4 + jsonLen) {
      return { isValid: false, error: `Invalid header size: expected ${4 + jsonLen}, got ${headerSize}` };
    }
    
    if (jsonLen <= 0 || jsonLen > 50 * 1024 * 1024) { // Sanity check: max 50MB header
...
function verifyHashConsistency(state: AsarState, context: AsarDebugContext): void {
  ...
  if (context.verifiedHash && state.jsonHeaderHash && context.verifiedHash !== state.jsonHeaderHash) {
    if (context.verifiedHash !== state.fullHeaderHash) {
      issues.push(`Verified hash ${context.verifiedHash} matches neither JSON (${state.jsonHeaderHash}) nor full (${state.fullHeaderHash}) hash`);
    }
  }
 ...
 */
export function logAsarState(checkpoint: string, asarPath: string, context: AsarDebugContext = {}): AsarState {
