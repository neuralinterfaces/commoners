...
function validateDependenciesForIntegrity(): boolean {
  ...
  if (!deps.ffiAvailable) {
    ...
    warn("For better performance and verification, consider installing: npm install ffi-napi ref-napi");
  }
...
function writeIntegrityResourceFFI(exePath: string, payloadJson: string) {
  ...
  for (const lang of languageIds) {
    ...
    if (!r) { 
      ...
      if (ok) { // Only set error on first failure
        ok = false;
        err = currentErr;
      }
    } else {
...
async function writeIntegrityResource(exePath: string, payloadJson: string) {
  ...
  const errorDetails = [
    ...
    ffiAttempted ? `- FFI method: ${ffiAttempted ? 'ATTEMPTED' : 'SKIPPED'} (${deps.ffiAvailable ? 'available' : 'unavailable'})` : null,
    rceditAttempted ? `- rcedit method: ${rceditAttempted ? 'ATTEMPTED' : 'SKIPPED'} (${deps.rceditAvailable ? 'available' : 'unavailable'})` : null,
...
export function readIntegrityResource(exePath: string) {
  ...
  try {
    ...
    if (!mod) {
      ...
      warn("This may indicate the executable is corrupted, locked, or has an incompatible architecture");
      return [];
    }
...
function looksLikeInstallerExe(p: string) {
  ...
  return n.endsWith(".exe") && (n.includes("setup") || n.includes("installer") || n.includes("nsis"));
}
...
function findMacAppForArtifact(artifactFile: string): string | null {
  ...
  for (const n of entries) {
    ...
    try {
      ...
      const s = lstatSync(p);
      if (s.size > max) { max = s.size; best = p; }
    } catch {}
  }
  return best;
}
  ...
  return async function afterPackOrArtifact(context: any & ArtifactCtx) {
    try {
      ...
      if (isAfterPack) {
        ...
        if (!product) { warn("No product name found, skipping integrity embedding"); return; }
        ...
        if (!file || !product) { warn("artifactBuildCompleted: missing file or product; skipping"); return; }
        ...
        if (platformName === "darwin" || process.platform === "darwin" || ext === ".app") {
          ...
          if (looksLikePortableExe(file)) {
            ...
            if (!unpacked) {
              warn("Windows installer artifact: could not locate 'win-unpacked'; will skip embedding (installer already built).");
              return;
            }
            appRoot = unpacked;
            asarPath = join(unpacked, "resources", "app.asar");
            exeForWin = findExe(unpacked, product);
            warn("Windows NSIS artifact: embedding into 'win-unpacked' will not affect this already-created installer.");
          } else {
            ...
            if (unpacked) {
              ...
              asarPath = join(unpacked, "resources", "app.asar");
              exeForWin = findExe(unpacked, product);
              warn("Windows artifact: updated sibling '*-unpacked' directory; current artifact unchanged.");
            } else {
              warn(`Windows artifact '${basename(file)}' is not a portable app and has no unpacked sibling; skipping.`);
              return;
            }
          }
        } else {
          // DMG/ZIP/AppImage/etc. — skip (read-only/archives)
          warn(`Artifact '${basename(file)}' is an archive/readonly format; skipping integrity embedding.`);
          return;
        }
      }
          ...
          try {
            ...
            if (first) {
              try {
                ...
                if (embedded === jsonHash) {
                  ...
                  logAsarState('AFTER_SUCCESSFUL_VERIFICATION', asarPath, { verifiedHash: embedded });
                  return;
                }
            ...
            try {
              ...
              if (embedded2 !== fullHash) {
                warn(`Failed to verify Windows integrity resource. Expected: ${fullHash}, got: ${embedded2}`);
