...
const merge = (
  ...
  for (const [k, v] of Object.entries(toMerge)) {
    ...
    if (opts.arrays && Array.isArray(updatedV) && Array.isArray(targetV))
      output[k] = [...targetV, ...updatedV] // Merge array entries together
    else if (isObject(updatedV) || isObject(targetV))
      output[k] = merge(v, targetV, opts, updatedPath) // Recurse into objects
