...
const getPreferredIcon = (
  icon: BaseIconType,
  preferences: IconFormatPreferences = WEB_FORMAT_PREFERENCE
) => {
...
export const getAllIcons = (icon: IconType) =>
  typeof icon === 'string' ? [icon] : Object.values(icon).flat() // Flatten in case of nesting
