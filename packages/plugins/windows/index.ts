import { type Plugin, type ElectronWindowOptions } from '@commoners/solidarity'
  ...
  const context = {
    ...
    on: (command, callback) =>
      eventTarget.addEventListener(
        'message',
        ev => ev.detail.command === command && callback(ev.detail.payload)
      ),
  }
...
const linkToMainElectronWindow = (id, context) => {
  ...
  return {
    ...
    send: (command, payload) =>
      readyPromise.then(() => context.send(`${id}:message`, { command, payload })), // Send message back to the main window
...
class BrowserWindow extends EventTarget {
  ...
  open = () => {
    ...
    const features = Object.entries(windowConfig)
    ...
    const ref = globalThis.open(pageUrl, this.id, features)
  ...
  return {
    ...
    ready: function () {
      ...
      this.on('open', async (_, type, requestId) => {
        ...
        const win = await createWindow(assets[type], window, onWindowCreation)
    ...
    desktop: {
      load: function (win) {
        ...
        if (__main)
          win.on('closed', () =>
            Object.values(this.WINDOWS).forEach(_win => _win !== win && _win.close())
          )
      },
