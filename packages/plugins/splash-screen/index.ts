  ...
  return {
    ...
    desktop: {
      load: async function (loadingWindow, pluginId) {
        ...
        const promiseToAwait = waitUntil
          ? Promise.all(
              Object.keys(win.__loading).map(
                id => waitUntil.includes(id) && id !== pluginId && win.__loading[id]
              )
            ).then(() => {})
          : loadingWindow.__ready
        ...
        if (minimumDisplayTime)
          setTimeout(show, minimumDisplayTime - (performance.now() - start)) // Show for duration left
        else show()
      },
    },
  }
